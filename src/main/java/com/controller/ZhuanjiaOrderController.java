    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(zhuanjiaOrder !=null){
            ...
            BeanUtils.copyProperties( zhuanjiaOrder , view );//把实体数据重构到view中
                ...
                //级联表
                HuanzheEntity huanzhe = huanzheService.selectById(zhuanjiaOrder.getHuanzheId());
                if(huanzhe != null){
    ...
    */
    ...
    public R save(@RequestBody ZhuanjiaOrderEntity zhuanjiaOrder, HttpServletRequest request){
        ...
        if(false)
            ...
            zhuanjiaOrder.setHuanzheId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
...
//                            zhuanjiaOrderEntity.setHuanzheId(Integer.valueOf(data.get(0)));   //患者 要改的
...
//                            zhuanjiaOrderEntity.setZhuanjiaOrderYuyueTypes(Integer.valueOf(data.get(0)));   //预约状态 要改的
    ...
    */
    ...
    public R add(@RequestBody ZhuanjiaOrderEntity zhuanjiaOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,zhuanjiaOrder:{}",this.getClass().getName(),zhuanjiaOrder.toString());
            ...
            double balance = huanzheEntity.getNewMoney() - zhuanjiaEntity.getGuahaoMoney();//余额
            if(balance<0)
                return R.error(511,"余额不够支付");
            zhuanjiaOrder.setHuanzheId(userId); //设置订单支付人id
            zhuanjiaOrder.setZhuanjiaOrderUuidNumber(String.valueOf(new Date().getTime()));
            zhuanjiaOrder.setZhuanjiaOrderYuyueTypes(1);
            zhuanjiaOrder.setInsertTime(new Date());
            zhuanjiaOrder.setCreateTime(new Date());
