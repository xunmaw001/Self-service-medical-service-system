...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的专家挂号订单 </p>
    </div>
    <!-- 标题 -->
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.huanzheName}}</td>
                            <td style="width: 60px;">{{item.zhuanjiaName}}</td>
                            <td style="width: 150px;">{{item.zhuanjiaOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.guahaoTime}}</td>
                            <td style="width: 80px;">{{item.shijianduanValue}}</td>
                            <td style="width: 80px;">{{item.zhuanjiaOrderYuyueValue}}</td>
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(zhuanjiaOrderTypes) {
                this.zhuanjiaOrderTypes = zhuanjiaOrderTypes;
                // 获取列表数据
                layui.http.request('zhuanjiaOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    zhuanjiaOrderTypes: (vue.zhuanjiaOrderTypes == 0)?null:vue.zhuanjiaOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('zhuanjiaOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    zhuanjiaOrderTypes: (this.zhuanjiaOrderTypes == 0)?null:this.zhuanjiaOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`zhuanjiaOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`zhuanjiaOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`zhuanjiaOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,commentback(id) {
                $("#id").val(id);//设置订单id
                $('#zhuanjiaCommentbackModal').modal('show');//打开模态框
            }
            ,submitZhuanjiaCommentback(){
                var zhuanjiaCommentbackContent = $("#zhuanjiaCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("zhuanjiaOrder/commentback?id="+id+"&commentbackText="+zhuanjiaCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function zhuanjiaOrderYuyueTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zhuanjia_order_yuyue_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('zhuanjiaOrder/page', 'get',{
            page: 1,
            limit: limit,
            zhuanjiaOrderTypes: (vue.zhuanjiaOrderTypes == 0)?null:vue.zhuanjiaOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('zhuanjiaOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            zhuanjiaOrderType: (vue.zhuanjiaOrderTypes == 0)?null:vue.zhuanjiaOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
